# Grab all USASpending.gov federal spending archive data since 2000.
# July 2013 was last time files from 2000 to 2002 were updated.
...
DIRDATE     <- "2013-07-15"            # yyyy-mm-dd format         ##### 3 of 4
FILEDATE    <-  gsub("-","",DIRDATE)   # yyyymmdd   format

# Grab all complete fiscal years and most recent incomplete year
FISCAL.YEAR <- 2013:2000    # Get more recent years first          ##### 4 of 4
...
for (i in 1:length(zip.files))
{
  ...
  if ( (!is.na(info$size)) & (info$size > 200) )  # skip 0-length zips
  {
